...
module PropertyTests =
    ...
    let ``monoid identity: empty combine dict = dict`` (keys: string list) (values: int list) =
        ...
        let pairs = List.zip uniqueKeys (values @ [for i in 1..List.length uniqueKeys -> 0] |> List.take (List.length uniqueKeys))
